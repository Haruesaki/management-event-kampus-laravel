...
<style>
  ...
  .page-top { display: flex; align-items: flex-start; gap: 24px; margin-bottom: 32px; }
  .page-top-left { flex: 1; }
  .page-title { font-family: 'Poppins', sans-serif; font-size: 40px; font-weight: 800; letter-spacing: -2px; line-height: 1; }
  ...
  .metric-card { background: var(--bg-card); border: 1px solid var(--border); border-radius: 14px; padding: 18px 22px; min-width: 160px; }
  ...
  .filter-tab { padding: 7px 18px; border-radius: 20px; font-size: 13px; font-weight: 500; cursor: pointer; border: 1px solid var(--border); color: var(--text-3); background: transparent; transition: all 0.2s; font-family: 'DM Sans', sans-serif; }
  ...
  .btn-onboard { display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; border-radius: 10px; background: rgba(124,92,252,0.15); border: 1px solid rgba(124,92,252,0.3); color: var(--accent-2); font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; font-family: 'DM Sans', sans-serif; text-decoration: none; }
  ...
  .table-row { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; padding: 16px 20px; border-bottom: 1px solid var(--border); align-items: center; transition: background 0.15s; cursor: pointer; }
  ...
  .alert-success { background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.2); color: #22c55e; padding: 12px 20px; border-radius: 12px; margin-bottom: 24px; font-size: 14px; font-weight: 500; }
  .alert-error { background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2); color: #ef4444; padding: 12px 20px; border-radius: 12px; margin-bottom: 24px; font-size: 14px; font-weight: 500; }

  .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.8); backdrop-filter: blur(10px); z-index: 9998; display: flex; align-items: center; justify-content: center; padding: 20px; }
  .insight-popup { position: relative; width: 100%; max-width: 440px; background: #15151e; border: 1px solid rgba(255,255,255,0.08); border-radius: 28px; padding: 40px; box-shadow: 0 40px 100px -20px rgba(0,0,0,0.7); z-index: 9999; }
  ...
  .popup-user { display: flex; flex-direction: column; align-items: center; text-align: center; margin-bottom: 28px; }
  ...
  .modal-btn-cancel:hover { border-color: rgba(255,255,255,0.15); color: #fff; }
  .modal-btn-danger {
    background: #ef4444;
  ...
  .modal-btn-ban:hover { background: rgba(239,68,68,0.2); }
  .modal-btn-unban {
    background: rgba(34,197,94,0.12);
    border: 1px solid rgba(34,197,94,0.3);
    color: #22c55e;
  }
  .modal-btn-unban:hover { background: rgba(34,197,94,0.2); }

  .warning-box {
    border-radius: 16px; padding: 18px 20px; margin-bottom: 24px;
  }
  .warning-box-red { background: rgba(239,68,68,0.06); border: 1px solid rgba(239,68,68,0.18); }
  .warning-box-title { font-size: 11px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 12px; }
  .warning-box-red .warning-box-title { color: #ef4444; }
  .warning-box-red ul li { color: rgba(239,68,68,0.85); }
  .warning-box ul { list-style: disc; padding-left: 18px; display: flex; flex-direction: column; gap: 8px; }
  .warning-box ul li { font-size: 13px; line-height: 1.5; }
</style>
@endpush
...
<div x-data="{
    ...
    openModal(user) { this.selectedUser = user; this.showModal = true; },
    openBanModal(user) { this.selectedUser = user; this.showBanModal = true; },
    openDeleteModal(user) { this.selectedUser = user; this.showDeleteModal = true; }
}">
    <div class="page-top">
    ...
    <div style="margin-bottom: 24px;">
        ...
        <div class="toolbar">
            ...
            <a href="{{ route('admin.users.create') }}" class="btn-onboard">
    ...
    <div class="table-wrapper">
        ...
        <template x-for="user in filteredUsers" :key="user.id">
            <div class="table-row" @click="openModal(user)">
                ...
                <div><span class="role-badge" :class="user.roleClass" x-text="user.role"></span></div>
    ...
    <div x-show="showBanModal" class="modal-overlay" style="display: none;" x-transition:enter="transition opacity duration-300" x-transition:leave="transition opacity duration-200">
        <div class="insight-popup" @click.away="showBanModal = false" x-transition:enter="transition transform duration-300" x-transition:enter-start="scale-95 opacity-0" x-transition:enter-end="scale-100 opacity-100">
            <div class="popup-header">
                <span class="popup-title" x-text="selectedUser && selectedUser.isActive ? 'Access Control — Restrict' : 'Access Control — Restore'"></span>
            ...
            <template x-if="selectedUser">
                <div>
                    ...
                    <!-- Status bar -->
                    <div style="display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.03); border: 1px solid var(--border); border-radius: 12px; padding: 14px 18px; margin-bottom: 20px;">
                        <div class="status-dot" :class="selectedUser.isActive ? 'dot-active' : 'dot-offline'"></div>
                        <span style="font-size: 13px; color: var(--text-2);">Status saat ini:</span>
                        <span style="font-size: 13px; font-weight: 700; margin-left: auto;"
                              :style="selectedUser.isActive ? 'color: #22c55e' : 'color: #8a8a9a'"
                              x-text="selectedUser.isActive ? 'Active — Authorized' : 'Inactive — Restricted'"></span>
                    </div>

                    <!-- Warning box -->
                    <div class="warning-box" :class="selectedUser.isActive ? 'warning-box-red' : ''"
                         :style="!selectedUser.isActive ? 'background: rgba(34,197,94,0.06); border: 1px solid rgba(34,197,94,0.18);' : ''">
                        <p class="warning-box-title" :style="selectedUser.isActive ? '' : 'color: #22c55e'">⚠ Peringatan Keamanan</p>
                        <ul>
                            <li :style="selectedUser.isActive ? 'color: rgba(239,68,68,0.85)' : 'color: rgba(34,197,94,0.85)'"
                                x-text="selectedUser.isActive ? 'Pengguna tidak akan bisa login ke sistem' : 'Pengguna akan mendapatkan kembali akses sistem'"></li>
                            <li :style="selectedUser.isActive ? 'color: rgba(239,68,68,0.85)' : 'color: rgba(34,197,94,0.85)'"
                                x-text="selectedUser.isActive ? 'Seluruh sesi aktif user akan segera berakhir' : 'User dapat kembali mengikuti event yang tersedia'"></li>
                            <li :style="selectedUser.isActive ? 'color: rgba(239,68,68,0.85)' : 'color: rgba(34,197,94,0.85)'">
                                Tindakan ini dapat dirubah kembali sewaktu-waktu</li>
                        </ul>
                    </div>
                    ...
                    <form :action="'/admin/users/' + selectedUser.id + '/toggle-status'" method="POST">
                        ...
                        <div style="display: flex; gap: 12px;">
                            <button type="button" class="modal-btn modal-btn-cancel" @click="showBanModal = false">Batal</button>
                            <button type="submit" class="modal-btn"
                                :class="selectedUser.isActive ? 'modal-btn-ban' : 'modal-btn-unban'"
                                x-text="selectedUser.isActive ? 'Ya, Banned Akun' : 'Ya, Aktifkan Akun'">
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <div x-show="showDeleteModal" class="modal-overlay" style="display: none;" x-transition:enter="transition opacity duration-300" x-transition:leave="transition opacity duration-200">
        <div class="insight-popup" @click.away="showDeleteModal = false" x-transition:enter="transition transform duration-300" x-transition:enter-start="scale-95 opacity-0" x-transition:enter-end="scale-100 opacity-100">
            <div class="popup-header">
                <span class="popup-title">Permanent Deletion</span>
            ...
            <template x-if="selectedUser">
                <div>
                    ...
                    <div class="warning-box warning-box-red">
                        <p class="warning-box-title">⚠ Peringatan Keamanan</p>
                        <ul>
                    ...
                    <form :action="'/admin/users/' + selectedUser.id" method="POST">
                        ...
                        @method('DELETE')
                        <div style="display: flex; gap: 12px;">
                            <button type="button" class="modal-btn modal-btn-cancel" @click="showDeleteModal = false">Batal</button>
                            <button type="submit" class="modal-btn modal-btn-danger">Ya, Hapus Akun</button>
                        </div>
                    </form>
                </div>
            </template>
